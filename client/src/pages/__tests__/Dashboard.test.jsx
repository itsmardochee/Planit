...
describe('Dashboard Page', () => {
  ...
  it('creates a new workspace successfully', async () => {
    ...
    vi.spyOn(apiModule.workspaceAPI, 'create').mockResolvedValue({
      data: { data: { _id: '3', name: 'New Workspace', description: 'New workspace' } },
    });
  ...
  it('cancels workspace creation', async () => {
    ...
    expect(screen.queryByPlaceholderText(/nom du workspace|workspace name/i)).not.toBeInTheDocument();
  });
  ...
  it('shows error when creating workspace fails', async () => {
    ...
    vi.spyOn(apiModule.workspaceAPI, 'create').mockRejectedValue(new Error('Failed to create'));
    ...
    await waitFor(() => {
      expect(screen.getByText(/erreur.*création.*workspace|error.*creating.*workspace/i)).toBeInTheDocument();
    });
  ...
  it('shows error when loading workspaces fails', async () => {
    vi.spyOn(apiModule.workspaceAPI, 'getAll').mockRejectedValue(new Error('Failed to load'));
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    ...
    await waitFor(() => {
      expect(screen.getByText(/erreur.*chargement.*workspaces|error.*loading.*workspaces/i)).toBeInTheDocument();
    });
  ...
  it('deletes workspace after confirmation', async () => {
    ...
    vi.spyOn(apiModule.workspaceAPI, 'delete').mockResolvedValue({ data: { success: true } });
  ...
  it('shows error when deleting workspace fails', async () => {
    ...
    vi.spyOn(apiModule.workspaceAPI, 'delete').mockRejectedValue(new Error('Failed to delete'));
    vi.stubGlobal('confirm', vi.fn(() => true));
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    ...
    await waitFor(() => {
      expect(screen.getByText(/erreur.*suppression.*workspace|error.*deleting.*workspace/i)).toBeInTheDocument();
    });
